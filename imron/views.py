from imroncart.forms import CartAddProductForm
from imroncart.keranjang import Cart
from django.shortcuts import render, get_object_or_404
from .models import Produk, Kategori, Kontak, Profil, Slide, Statis, ChatID
from imroncart.models import Transaksi, DetailTransaksi
...
class KontakView(View):
    ...
    def post(self, request):
        ...
        kontak = Kontak.objects.create(nama=nama, email=email,
                                       no_whatsup=no_whatsup, subject=subject, isi=pesan)
        kontak.save()
        context = {
...
class CheckoutView(View):
    ...
    def post(self, request):
        ...
        if grantotal == "0":
            messages.error(
                request, 'Anda belum berbelanja, Silakan belanja terlebih dahulu')
            context['has_error'] = True
        if nama_depan == "":
...
def cari(request):
    ...
    imroncart_produk_form = CartAddProductForm()
    jmlproduk = hasilcari.count()
    context = {
        "judul": "Halaman Cari",
        "cart product form": imroncart_produk_form,
        "hasilcari": hasilcari,
        "Jmlproduk" :jmlproduk 
    }
    print(hasilcari)
